...
By invoking the I<file-chooser:choose> action, the scrollback buffer is scanned for
paths to files and a list is presented with the result.
This list can be navigated and the application specified by the
I<file-chooser.launcher> resource is used to open the desired file.
...
syntax is not understood by every editor, the I<file-chooser.launcher> resource
...
Because of this, the B<file-chooser> does not work while programs like B<less>
are in the foreground, if the launcher is a command line editor.

The length of the result list is limited by the resource
I<file-chooser.result-length>, which specifies its maximum length.

In the result list, a cursor symbol is used to indicate the currently selected
path. This symbol can be changed by using the I<file-chooser.indicator> resource.
...
for each resource entry:
    ...
    URxvt.file-chooser.result-length: 5
    URxvt.file-chooser.cursor: ►

Enable the extension and configure a keysym for the I<file-chooser:choose> action:

    URxvt.perl-ext-common: default,file-chooser,...
    URxvt.keysym.M-c: file-chooser:choose
...
The following shows examples of launcher resources for different types of editors/IDEs.
    ...
    URxvt.file-chooser.launcher: @jed '%1$s' -g '%2$s'
    URxvt.file-chooser.launcher: @vim '+normal %2$sG%3$s|' '%1$s'
    ## GUI editors / IDEs
    URxvt.file-chooser.launcher: kate '%1$s:%2$s:%3$s'
    URxvt.file-chooser.launcher: gedit '%1$s' '+%2$s:%3$s'
    URxvt.file-chooser.launcher: atom '%1$s:%2$s:%3$s'
    URxvt.file-chooser.launcher: gvim '+normal %2$sG%3$s|' '%1$s'
    URxvt.file-chooser.launcher: emacsclient -n '+%2$s:%3$s' '%1$s'
    URxvt.file-chooser.launcher: geany '--line=%2$s' '--column=%3$s' '%1$s'
